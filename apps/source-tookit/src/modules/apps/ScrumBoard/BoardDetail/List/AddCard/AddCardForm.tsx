...
import type { AuthUserType } from '@crema/models/AuthUser';
...
const AddCardForm: React.FC<AddCardFormProps> = ({
  ...
  const labelList = useAppSelector(
    ({ scrumboardApp }) => scrumboardApp.labelList
  );
  ...
  const onFinish = (values: any) => {
    ...
    if (selectedCard) {
      ...
      if (list) dispatch(onAddNewCard(board, list, newCard));
    }
  };
